...
ASSIGNMENT: spark sp-ux0011. You have been assigned this spark. Mark it in progress now: `ryve spark status sp-ux0011 in_progress`

Title: Error toast notification system

Description:
All errors go to log::error and are invisible. Need dismissible toast notifications for: DB init failure, download errors, worktree creation failure, file load errors. Currently background download failure leaves picker in permanent loading state (actual bug).
