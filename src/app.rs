...
use crate::font_intern;
use crate::process_snapshot::{self, ProcessSnapshot};
...
use crate::workshop::{SparkPatch, Workshop};
...
impl App {
    ...
    fn view_workshop<'a>(&'a self, ws: &'a Workshop) -> Element<'a, Message> {
        ...
        let sidebar = self.view_sidebar(ws, has_bg, &pal);
        let bench = self.view_bench(ws, has_bg, &pal);
        let delegation = self.build_delegation_trace(ws);
        let sparks_panel = self.view_sparks_panel(ws, &delegation, &pal, has_bg);
        let sparks_col = container(sparks_panel)
            .width(ws.sparks_width())
            .height(Length::Fill);
        let status_bar = self.view_status_bar(ws, &pal, has_bg);

        let sidebar_bench_splitter = crate::widget::splitter::vertical(
            Message::SplitterPressed(SplitterKind::SidebarRight),
            &pal,
        );
        let bench_sparks_splitter = crate::widget::splitter::vertical(
        ...
        self.push_background_layers(&mut layers, ws);
        layers.push(workshop_content);
        self.push_modal_overlays(&mut layers, ws, &pal);
    ...
    fn view_sidebar<'a>(
        ...
        let agents_panel = container(self.view_agents(ws, has_bg, pal))
            ...
            .style(move |_theme: &Theme| style::glass_panel(pal, has_bg));
        let splitter = crate::widget::splitter::horizontal(
            Message::SplitterPressed(SplitterKind::SidebarFilesHands),
            pal,
        );
        column![files_panel, splitter, agents_panel]
            .spacing(0)
    ...
    fn build_delegation_trace(&self, ws: &Workshop) -> screen::delegation_trace::DelegationTrace {
        ws.selected_spark
            .as_deref()
            .map(|selected_id| {
    ...
    fn view_sparks_panel<'a>(
        &'a self,
        ws: &'a Workshop,
        delegation: &'a screen::delegation_trace::DelegationTrace,
        pal: &style::Palette,
        has_bg: bool,
    ) -> Element<'a, Message> {
        if ws.show_releases {
            return screen::releases::view(&ws.release_view_data, &ws.releases_state, pal, has_bg)
        ...
        if let Some(ref selected_id) = ws.selected_spark {
            if let Some(spark) = ws.sparks.iter().find(|s| s.id == *selected_id) {
                let description_draft = ws
                    ...
                    .map(|s| s.as_str());
                return screen::spark_detail::view(
                    spark,
        ...
        self.view_sparks_list(ws, *pal, has_bg)
    }
    ...
    fn view_status_bar<'a>(
        ...
        let file_info = ws.bench.active_tab.and_then(|tab_id| {
            ...
            let (line, col) = viewer.cursor_position();
